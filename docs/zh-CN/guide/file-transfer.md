...
`cp` 是唯一忽略全局 `--timeout` 默认值 `30s` 的命令。传输时长由文件大小和链路速度决定，固定的截止时间起不到超时的作用，只会变成体积上限——在 1.4 MB/s 的链路上，30 秒的上限把传输卡在约 48 MB。
...
超时返回 `error.code: "timeout"`，目标端的部分临时文件会被清理。

传输卡死没有任何保护。远端在传输途中停止响应时，`cp` 会无限等待，`--timeout` 也救不了：截止时间在数据块之间检查，而卡住的读取永远到不了下一个数据块。挂起会阻塞流水线时，在外层套超时：
