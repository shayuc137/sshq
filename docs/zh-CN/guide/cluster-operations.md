...
解析主机选择器后，sshq 在执行前对所有目标主机进行策略检查。任何主机被策略拦截，所有主机都不执行——避免部分主机执行了本应被限制的命令。

策略预检和运行时部分失败是不同的：预检干净地拒绝整个操作，运行时失败是逐主机独立的。

## 错误处理

运行时失败不会阻止其他主机。每台被选中的主机都有独立结果。任意主机连接失败、执行失败或远端退出码非零，本地命令都会返回非零退出码。
